    ...
    init(setEdgeState, orientation) {
        ...
        G6.registerEdge('customEdge', {
            draw(cfg: ModelConfig & IEdgeConfig, group: IGroup): IShape {
                ...
                let sourceBoxBound = (cfg.source as Item).getBBox();
                let targetBoxBound = (cfg.target as Item).getBBox();
                ...
                if (!(cfg.target as Position).x) {
                    ...
                    if (orientation === 'vertical') {
                            ...
                            y:
                                targetNode.y +
                                portXY(
                                    targetBoxBound.height,
                                    cfg.inputPortIndex,
                                    targetNode.component.inputs.length,
                                    orientation,
                                    'y'
                                ),
                        };
                ...
                let hgap = Math.abs(end.x - start.x) || 0.1;
                let path = [];
                if (hgap === 0) {
                    path = [
                        ['M', start.x, start.y],
